...
const Test = decodeType(
  "Test",
  {
    ...
    Record: decodeVariant(
      VariantRecord,
      {
        name: decodeString(Ok, Err),
        age: decodeNumber(Ok, Err),
      },
      Ok,
      Err,
    ),
...
test("OK decodeType (record variant)", () => {
  const decoded = Test({ type: "Record", value: { name: "Joe", age: 100 } });
...
test("FAIL decodeType (params variant)", () => {
  const decoded = Test({ type: "Record", value: { name: true, age: 100 } });
