    ...
    module Comparer
      ...
      def unify(node1, node2)
        ...
        when node2.is_a?(Variable)
          unify(node2, node1)
        when node1.is_a?(Tag) && node2.is_a?(Tag)
          raise "Not unified" unless node1.name == node2.name
          node1
        when node1.is_a?(Tags) && node2.is_a?(Tag)
          raise "Not unified!" unless node1.options.includes?(node2.name)
          node1
        when node1.is_a?(Tag) && node2.is_a?(Tags)
          unify(node2, node1)
      ...
      def prune(node : Record | Tag | Tags)
        node
      end
    end
  end
