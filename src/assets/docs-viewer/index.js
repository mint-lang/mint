...
(() => {
  ...
  class $$Page_Dashboard extends Enum {
  constructor() {
    super()
...
class $$Page_Package extends Enum {
  constructor() {
    super()
...
class $$Page_Entity extends Enum {
  constructor() {
    super()
...
class $$Type_Component extends Enum {
  constructor() {
    super()
...
class $$Type_Provider extends Enum {
  constructor() {
    super()
...
class $$Type_Record extends Enum {
  constructor() {
    super()
...
class $$Type_Module extends Enum {
  constructor() {
    super()
...
class $$Status_DecodeError extends Enum {
  constructor() {
    super()
...
class $$Status_HttpError extends Enum {
  constructor() {
    super()
...
class $$Status_JsonError extends Enum {
  constructor() {
    super()
...
class $$Status_Initial extends Enum {
  constructor() {
    super()
...
$$Property.mappings = {"defaultValue":"default","description":null,"name":null,"type":null}

$$Property.decode = (_input) => {
  let defaultValue = Decoder.field(`default`, Decoder.string)(_input)
  if (defaultValue instanceof Err) { return defaultValue }

  let description = Decoder.field(`description`, Decoder.maybe(Decoder.string))(_input)
  ...
  let type = Decoder.field(`type`, Decoder.string)(_input)
  if (type instanceof Err) { return type }

  return new Ok(new $$Property({
...
class $$ComputedProperty extends Record {}

$$ComputedProperty.mappings = {"description":null,"source":null,"name":null,"type":null}

$$ComputedProperty.decode = (_input) => {
  let description = Decoder.field(`description`, Decoder.maybe(Decoder.string))(_input)
  if (description instanceof Err) { return description }

  let source = Decoder.field(`source`, Decoder.string)(_input)
  ...
  return new Ok(new $$ComputedProperty({
    description: description.value,
    source: source.value,
    name: name.value,
    type: type.value
...
class $$Connect extends Record {}

$$Connect.mappings = {"keys":null,"store":null}

$$Connect.decode = (_input) => {
  let keys = Decoder.field(`keys`, Decoder.array(Decoder.string))(_input)
  if (keys instanceof Err) { return keys }

  let store = Decoder.field(`store`, Decoder.string)(_input)
...
$$Component.mappings = {"computedProperties":"computed-properties","states":"states","properties":null,"description":null,"connects":null,"functions":null,"providers":null,"name":null}

$$Component.decode = (_input) => {
  let computedProperties = Decoder.field(`computed-properties`, Decoder.array($$ComputedProperty.decode))(_input)
  if (computedProperties instanceof Err) { return computedProperties }

  let states = Decoder.field(`states`, Decoder.array($$Property.decode))(_input)
  if (states instanceof Err) { return states }

  let properties = Decoder.field(`properties`, Decoder.array($$Property.decode))(_input)
  if (properties instanceof Err) { return properties }

  let description = Decoder.field(`description`, Decoder.maybe(Decoder.string))(_input)
  if (description instanceof Err) { return description }

  let connects = Decoder.field(`connects`, Decoder.array($$Connect.decode))(_input)
  if (connects instanceof Err) { return connects }

  let functions = Decoder.field(`functions`, Decoder.array($$Method.decode))(_input)
  if (functions instanceof Err) { return functions }

  let providers = Decoder.field(`providers`, Decoder.array($$Use.decode))(_input)
  if (providers instanceof Err) { return providers }

  let name = Decoder.field(`name`, Decoder.string)(_input)
  if (name instanceof Err) { return name }

  return new Ok(new $$Component({
...
class $$Use extends Record {}

$$Use.mappings = {"condition":null,"provider":null,"data":null}

$$Use.decode = (_input) => {
  let condition = Decoder.field(`condition`, Decoder.maybe(Decoder.string))(_input)
  if (condition instanceof Err) { return condition }

  let provider = Decoder.field(`provider`, Decoder.string)(_input)
  if (provider instanceof Err) { return provider }

  let data = Decoder.field(`data`, Decoder.string)(_input)
  if (data instanceof Err) { return data }

  return new Ok(new $$Use({
...
$$Store.mappings = {"computedProperties":"computed-properties","states":"states","description":null,"functions":null,"name":null}
...
$$Method.mappings = {"arguments":null,"description":null,"source":null,"name":null,"type":null}

$$Method.decode = (_input) => {
  let arguments = Decoder.field(`arguments`, Decoder.array($$Argument.decode))(_input)
...
$$Provider.mappings = {"description":null,"functions":null,"subscription":null,"name":null}

$$Provider.decode = (_input) => {
  let description = Decoder.field(`description`, Decoder.maybe(Decoder.string))(_input)
  if (description instanceof Err) { return description }
  ...
  let subscription = Decoder.field(`subscription`, Decoder.string)(_input)
  if (subscription instanceof Err) { return subscription }

  let name = Decoder.field(`name`, Decoder.string)(_input)
  if (name instanceof Err) { return name }

  return new Ok(new $$Provider({
    description: description.value,
    functions: functions.value,
...
$$Argument.mappings = {"name":null,"type":null}

$$Argument.decode = (_input) => {
  let name = Decoder.field(`name`, Decoder.string)(_input)
  if (name instanceof Err) { return name }
...
$$Module.mappings = {"description":null,"functions":null,"name":null}

$$Module.decode = (_input) => {
  let description = Decoder.field(`description`, Decoder.maybe(Decoder.string))(_input)
  if (description instanceof Err) { return description }
...
$$Content.mappings = {"computedProperties":null,"properties":null,"fields":null,"options":null,"parameters":null,"connects":null,"functions":null,"states":null,"subscription":null,"description":null,"uses":null,"name":null}

$$Content.decode = (_input) => {
  let computedProperties = Decoder.field(`computedProperties`, Decoder.array($$ComputedProperty.decode))(_input)
  if (computedProperties instanceof Err) { return computedProperties }

  let properties = Decoder.field(`properties`, Decoder.array($$Property.decode))(_input)
  if (properties instanceof Err) { return properties }

  let fields = Decoder.field(`fields`, Decoder.array($$RecordField.decode))(_input)
  if (fields instanceof Err) { return fields }

  let options = Decoder.field(`options`, Decoder.array($$EnumOption.decode))(_input)
  if (options instanceof Err) { return options }

  let parameters = Decoder.field(`parameters`, Decoder.array(Decoder.string))(_input)
  if (parameters instanceof Err) { return parameters }
  ...
  let uses = Decoder.field(`uses`, Decoder.array($$Use.decode))(_input)
  if (uses instanceof Err) { return uses }

  let name = Decoder.field(`name`, Decoder.string)(_input)
  if (name instanceof Err) { return name }

  return new Ok(new $$Content({
    computedProperties: computedProperties.value,
    properties: properties.value,
    fields: fields.value,
...
class $$Documentation extends Record {}

$$Documentation.mappings = {"dependencies":null,"components":null,"providers":null,"records":null,"modules":null,"stores":null,"enums":null,"name":null}

$$Documentation.decode = (_input) => {
  let dependencies = Decoder.field(`dependencies`, Decoder.array($$Dependency.decode))(_input)
  if (dependencies instanceof Err) { return dependencies }

  let components = Decoder.field(`components`, Decoder.array($$Component.decode))(_input)
  if (components instanceof Err) { return components }

  let providers = Decoder.field(`providers`, Decoder.array($$Provider.decode))(_input)
  if (providers instanceof Err) { return providers }

  let records = Decoder.field(`records`, Decoder.array($$Record.decode))(_input)
  if (records instanceof Err) { return records }

  let modules = Decoder.field(`modules`, Decoder.array($$Module.decode))(_input)
  if (modules instanceof Err) { return modules }

  let stores = Decoder.field(`stores`, Decoder.array($$Store.decode))(_input)
  if (stores instanceof Err) { return stores }

  let enums = Decoder.field(`enums`, Decoder.array($$Enum.decode))(_input)
  if (enums instanceof Err) { return enums }

  let name = Decoder.field(`name`, Decoder.string)(_input)
  if (name instanceof Err) { return name }

  return new Ok(new $$Documentation({
...
$$Root.mappings = {"packages":null}

$$Root.decode = (_input) => {
  let packages = Decoder.field(`packages`, Decoder.array($$Documentation.decode))(_input)
  if (packages instanceof Err) { return packages }

  return new Ok(new $$Root({
...
$$Dependency.mappings = {"repository":null,"constraint":null,"name":null}

$$Dependency.decode = (_input) => {
  let repository = Decoder.field(`repository`, Decoder.string)(_input)
  if (repository instanceof Err) { return repository }

  let constraint = Decoder.field(`constraint`, Decoder.string)(_input)
  if (constraint instanceof Err) { return constraint }

  let name = Decoder.field(`name`, Decoder.string)(_input)
  if (name instanceof Err) { return name }

  return new Ok(new $$Dependency({
...
class $$RecordField extends Record {}

$$RecordField.mappings = {"mapping":null,"type":null,"key":null}

$$RecordField.decode = (_input) => {
  let mapping = Decoder.field(`mapping`, Decoder.maybe(Decoder.string))(_input)
  if (mapping instanceof Err) { return mapping }

  let type = Decoder.field(`type`, Decoder.string)(_input)
  if (type instanceof Err) { return type }

  let key = Decoder.field(`key`, Decoder.string)(_input)
  if (key instanceof Err) { return key }

  return new Ok(new $$RecordField({
...
$$Record.mappings = {"fields":null,"description":null,"name":null}
...
class $$Enum extends Record {}

$$Enum.mappings = {"description":null,"options":null,"parameters":null,"name":null}
...
class $$EnumOption extends Record {}

$$EnumOption.mappings = {"description":null,"parameters":null,"name":null}

$$EnumOption.decode = (_input) => {
  let description = Decoder.field(`description`, Decoder.maybe(Decoder.string))(_input)
  if (description instanceof Err) { return description }
    ...
    return (() => {
      let __condition = input
       ...
       if (_compare(__condition, `component`)) {
        return $Result.ok(new $$Type_Component())
      } else if (_compare(__condition, `provider`)) {
        return $Result.ok(new $$Type_Provider())
      } else if (_compare(__condition, `record`)) {
        return $Result.ok(new $$Type_Record())
      } else if (_compare(__condition, `module`)) {
        return $Result.ok(new $$Type_Module())
      } else if (_compare(__condition, `store`)) {
        return $Result.ok(new $$Type_Store())
      } else if (_compare(__condition, `enum`)) {
        return $Result.ok(new $$Type_Enum())
      } else {
  ...
  char(type) {
    return (() => {
      let __condition = type
         ...
         if (__condition instanceof $$Type_Component) {
          

          return `C`
        }   if (__condition instanceof $$Type_Provider) {
          

          return `P`
        }   if (__condition instanceof $$Type_Record) {
          

          return `R`
        }   if (__condition instanceof $$Type_Module) {
          

          return `M`
        }   if (__condition instanceof $$Type_Store) {
          

          return `S`
        }   if (__condition instanceof $$Type_Enum) {
...
const $Application = new (class extends Store {
    ...
    status: new $$Status_Initial(),
    tab: new $$Type_Component(),
    documentations: [],
    documentation: $Documentation.empty(),
    page: new $$Page_Dashboard()
    }
  }
  ...
  get tab() { return this.state.tab }

  get documentations() { return this.state.documentations }

  get documentation() { return this.state.documentation }

  get page() { return this.state.page }

  load() {
    return (_compare(this.status, new $$Status_Initial()) ? (async () => {
      let _result = null;
  ...
  dashboard() {
    return (async () => {
      ...
      _result = await $Window.setScrollTop(0)
      }
      catch (_error) {
        if (_error instanceof DoError) {} else {
            console.warn(`Unhandled error in sequence expression:`)
  ...
  routePackage(name) {
    return (async () => {
      ...
      if (_1 instanceof Err) {
       ...
       _result = $Window.navigate(`/`)

      throw new DoError()
      }

      let nextDocumentation = _1.value

       await new Promise((_resolve) => {
        this.setState(_update(this.state, new Record({
      documentation: nextDocumentation,
      page: new $$Page_Package()
      })), _resolve)
      })
  ...
  route(packageName, tabName, entity) {
    return (async () => {
      ...
      _result = await (async () => {
        ...
        let nextTab = _0.value

        let items = await (() => {
          let __condition = nextTab

             if (__condition instanceof $$Type_Component) {
              

              return $Array.map($Content.fromComponent.bind($Content), nextDocumentation.components)
            }   if (__condition instanceof $$Type_Provider) {
              

              return $Array.map($Content.fromProvider.bind($Content), nextDocumentation.providers)
            }   if (__condition instanceof $$Type_Record) {
              

              return $Array.map($Content.fromRecord.bind($Content), nextDocumentation.records)
            }   if (__condition instanceof $$Type_Module) {
              

              return $Array.map($Content.fromModule.bind($Content), nextDocumentation.modules)
            }   if (__condition instanceof $$Type_Store) {
              

              return $Array.map($Content.fromStore.bind($Content), nextDocumentation.stores)
            }   if (__condition instanceof $$Type_Enum) {
              

              return $Array.map($Content.fromEnum.bind($Content), nextDocumentation.enums)
            }
        })()
        ...
        _result = await (async () => {
          ...
          _result = await $Window.navigate(`/` + nextDocumentation.name + `/` + $Type.path(nextTab) + `/` + first.name)
          }
          catch (_error) {
            if (_error instanceof DoError) {} else {
              _result = $Window.navigate(`/` + nextDocumentation.name)
            }
          } 
  ...
  get content() {
    return (() => {
      let __condition = this.page
         ...
         if (__condition instanceof $$Page_Dashboard) {
          

          return _createElement($Dashboard, {  })
        }   if (__condition instanceof $$Page_Package) {
          

          return _createElement($Package, {  })
        }   if (__condition instanceof $$Page_Entity) {
...
class $Sidebar extends Component {
  get items() {
    return (() => {
      ...
      return _createElement($Sidebar_Item, { "type": new $$Type_Component(), "text": item.name })
      }), this.documentation.components)
        }   if (__condition instanceof $$Type_Provider) {
          

          return $Array.map(((item) => {
      return _createElement($Sidebar_Item, { "type": new $$Type_Provider(), "text": item.name })
      }), this.documentation.providers)
        }   if (__condition instanceof $$Type_Store) {
          

          return $Array.map(((item) => {
      return _createElement($Sidebar_Item, { "type": new $$Type_Store(), "text": item.name })
      }), this.documentation.stores)
        }   if (__condition instanceof $$Type_Record) {
          

          return $Array.map(((item) => {
      return _createElement($Sidebar_Item, { "type": new $$Type_Record(), "text": item.name })
      }), this.documentation.records)
        }   if (__condition instanceof $$Type_Module) {
          

          return $Array.map(((item) => {
      return _createElement($Sidebar_Item, { "type": new $$Type_Module(), "text": item.name })
      }), this.documentation.modules)
        }   if (__condition instanceof $$Type_Enum) {
          

          return $Array.map(((item) => {
      return _createElement($Sidebar_Item, { "type": new $$Type_Enum(), "text": item.name })
...
class $Source extends Component {
  ...
  get text() {
    return (this.shown ? `Hide source ` : `Show source`)
  }

  get transform() {
    return (this.shown ? `rotate(90deg)` : ``)
  }
  ...
  get shown() { return this.state.shown }
  ...
  render() {
    ...
    }, [this.icon, _createElement("div", {}, [this.text])]), _createElement($If, { "condition": this.shown }, _array(_createElement("div", {
...
class $Tabs extends Component {
  get isDashboard() {
    return _compare(this.page, new $$Page_Dashboard())
  }
  ...
  render() {
    ...
    }, [_createElement($Tab, { "active": _compare(this.page, new $$Page_Dashboard()), "icon": this.dashboardIcon, "color": `#666`, "link": `/` }), _createElement($If, { "condition": !_compare(this.documentation.name, ``) }, _array(_createElement($Tab, { "active": _compare(this.page, new $$Page_Package()), "link": `/` + this.documentation.name, "title": this.documentation.name, "icon": $Icons.package(), "color": `#666` }))), _createElement($Unless, { "condition": $Array.isEmpty(this.documentation.components) }, _array(_createElement($EntityTab, { "of": new $$Type_Component() }))), _createElement($Unless, { "condition": $Array.isEmpty(this.documentation.modules) }, _array(_createElement($EntityTab, { "of": new $$Type_Module() }))), _createElement($Unless, { "condition": $Array.isEmpty(this.documentation.stores) }, _array(_createElement($EntityTab, { "of": new $$Type_Store() }))), _createElement($Unless, { "condition": $Array.isEmpty(this.documentation.providers) }, _array(_createElement($EntityTab, { "of": new $$Type_Provider() }))), _createElement($Unless, { "condition": $Array.isEmpty(this.documentation.records) }, _array(_createElement($EntityTab, { "of": new $$Type_Record() }))), _createElement($Unless, { "condition": $Array.isEmpty(this.documentation.enums) }, _array(_createElement($EntityTab, { "of": new $$Type_Enum() })))])
  }
}
...
class $State extends Component {
  ...
  render() {
    ...
    }, [_createElement($Pre, { "code": this.defaultValue })])])
  }
}
...
class $EntityTab extends Component {
  ...
  render() {
    return _createElement($Tab, { "link": `/` + this.documentation.name + `/` + $Type.path(this.of), "active": _compare(this.of, this.tab) && _compare(this.page, new $$Page_Entity()), "title": $Type.title(this.of), "color": $Type.color(this.of), "icon": $Type.icon(this.of) })
  }
}
...
class $Option extends Component {
  ...
  render() {
    ...
    }, [this.name, _createElement($Unless, { "condition": $Array.isEmpty(this.parameters) }, _array(_createElement("div", {
      className: `option-parameters`
    }, [$String.join(`, `, this.parameters)])))]), _createElement($If, { "condition": $Maybe.isJust(this.description) }, _array(_createElement("div", {
...
class $Page extends Component {
  ...
  render() {
    ...
    let states = $Array.map(((item) => {
    return _createElement($Entity, { "key": this.selected.name + item.name, "defaultValue": item.defaultValue, "description": item.description, "name": item.name, "type": item.type })
    }), this.selected.states)

    let options = $Array.map(((item) => {
    return _createElement($Option, { "description": item.description, "parameters": item.parameters, "name": item.name })
    ...
    }, [this.selected.name, _createElement($Unless, { "condition": $Array.isEmpty(this.selected.parameters) }, _array(_createElement("div", {
      className: `page-parameters`
    }, [$String.join(`, `, this.selected.parameters)])))]), _createElement("div", {
    ...
    }, [`Connected Stores`]), _createElement("div", {}, [connects]))), _createElement($Unless, { "condition": $Array.isEmpty(states) }, _array(_createElement("div", {
      className: `page-section`
    }, [`States`]), _createElement("div", {}, [states]))), _createElement($Unless, { "condition": $String.isEmpty(this.selected.subscription) }, _array(_createElement("div", {
      className: `page-section`
...
_insertStyles(`
  ...
  .option-parameters::before {
    content: "(";
    color: #333;
  }

  .option-parameters::after {
    content: ")";
    color: #333;
  }
  ...
  .page-parameters::before {
    content: "(";
    color: #333;
  }

  .page-parameters::after {
    content: ")";
    color: #333;
  }
`)
