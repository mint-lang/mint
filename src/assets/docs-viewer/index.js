...
$$Component.decode = (input) => {
  let computedProperties = Decoder.field(`computed-properties`, Decoder.array($$ComputedProperty.decode))(input)
  if (computedProperties instanceof Err) { return computedProperties }

  let states = Decoder.field(`states`, Decoder.array($$Property.decode))(input)
  if (states instanceof Err) { return states }

  let properties = Decoder.field(`properties`, Decoder.array($$Property.decode))(input)
  if (properties instanceof Err) { return properties }

  let description = Decoder.field(`description`, Decoder.maybe(Decoder.string))(input)
  if (description instanceof Err) { return description }

  let connects = Decoder.field(`connects`, Decoder.array($$Connect.decode))(input)
  if (connects instanceof Err) { return connects }

  let functions = Decoder.field(`functions`, Decoder.array($$Method.decode))(input)
  if (functions instanceof Err) { return functions }

  let providers = Decoder.field(`providers`, Decoder.array($$Use.decode))(input)
...
$$Method.decode = (input) => {
  let arguments = Decoder.field(`arguments`, Decoder.array($$Argument.decode))(input)
...
$$Content.mappings = {"computedProperties":null,"properties":null,"fields":null,"options":null,"connects":null,"functions":null,"states":null,"subscription":null,"description":null,"uses":null,"name":null}

$$Content.decode = (input) => {
  let computedProperties = Decoder.field(`computedProperties`, Decoder.array($$ComputedProperty.decode))(input)
  if (computedProperties instanceof Err) { return computedProperties }

  let properties = Decoder.field(`properties`, Decoder.array($$Property.decode))(input)
  if (properties instanceof Err) { return properties }

  let fields = Decoder.field(`fields`, Decoder.array($$RecordField.decode))(input)
  if (fields instanceof Err) { return fields }

  let options = Decoder.field(`options`, Decoder.array($$EnumOption.decode))(input)
  ...
  let description = Decoder.field(`description`, Decoder.string)(input)
  if (description instanceof Err) { return description }

  let uses = Decoder.field(`uses`, Decoder.array($$Use.decode))(input)
...
$$Documentation.decode = (input) => {
  let dependencies = Decoder.field(`dependencies`, Decoder.array($$Dependency.decode))(input)
  if (dependencies instanceof Err) { return dependencies }

  let components = Decoder.field(`components`, Decoder.array($$Component.decode))(input)
  if (components instanceof Err) { return components }

  let providers = Decoder.field(`providers`, Decoder.array($$Provider.decode))(input)
  if (providers instanceof Err) { return providers }

  let records = Decoder.field(`records`, Decoder.array($$Record.decode))(input)
  if (records instanceof Err) { return records }

  let modules = Decoder.field(`modules`, Decoder.array($$Module.decode))(input)
  if (modules instanceof Err) { return modules }

  let stores = Decoder.field(`stores`, Decoder.array($$Store.decode))(input)
  if (stores instanceof Err) { return stores }

  let enums = Decoder.field(`enums`, Decoder.array($$Enum.decode))(input)
...
$$Root.decode = (input) => {
  let packages = Decoder.field(`packages`, Decoder.array($$Documentation.decode))(input)
...
$$Enum.mappings = {"description":null,"options":null,"name":null}
...
$$EnumOption.mappings = {"description":null,"name":null}

$$EnumOption.decode = (input) => {
  let description = Decoder.field(`description`, Decoder.maybe(Decoder.string))(input)
  if (description instanceof Err) { return description }
...
const $Application = new (class extends Store {
  ...
  load() {
    ...
    if (_2 instanceof Err) {
     ...
     (async () => {
      try {
         await $Debug.log($Object_Error.toString(error))

     await new Promise((_resolve) => {
      this.setState(_update(this.state, new Record({
    status: $Status_DecodeError
    })), _resolve)
    })
      }
      catch(_error) {
